...
return new class extends Migration {
    ...
    {
        Schema::create('tfp_localizer_records', function (Blueprint $table) {
            $table->id();
            $table->string('form_number', 80)->unique();
            $table->string('form_type', 30)->default('LOCALIZER');
            $table->date('date');
            $table->string('day_name', 20)->nullable();
            $table->string('time_filled', 10)->nullable();
            $table->string('shift_type', 10);
            $table->string('location', 100)->default('GEDUNG LOCALIZER');
        ...
        \Illuminate\Support\Facades\DB::statement(
            'CREATE UNIQUE INDEX tfp_localizer_records_unique_form_per_shift ON tfp_localizer_records (form_type, date, shift_type) WHERE deleted_at IS NULL'
        );
    }
    public function down(): void
    {
        \Illuminate\Support\Facades\DB::statement('DROP INDEX IF EXISTS tfp_localizer_records_unique_form_per_shift');
        Schema::dropIfExists('tfp_localizer_records');
    }
};
