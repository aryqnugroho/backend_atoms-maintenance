...
/**
 * cnsd_asmgcs_meter_records — header for CNSD ASMGCS Meter Reading (SAAB).
 *
 * Defaults: merk=SAAB, type=null, sn=null per the paper-form header.
 */
return new class extends Migration {
    public function up(): void
    {
        Schema::create('cnsd_asmgcs_meter_records', function (Blueprint $table) {
            $table->id();

            $table->string('form_number', 80)->unique();
            $table->string('form_type', 30)->default('ASMGCS-METER');
            $table->string('facility', 20)->default('ASMGCS');
            ...
            $table->string('merk', 60)->default('SAAB');
        ...
        \Illuminate\Support\Facades\DB::statement(
            'CREATE UNIQUE INDEX cnsd_asmgcs_meter_records_unique_form_per_shift '
            . 'ON cnsd_asmgcs_meter_records (form_type, facility, date, shift_type) '
    ...
    {
        \Illuminate\Support\Facades\DB::statement(
            'DROP INDEX IF EXISTS cnsd_asmgcs_meter_records_unique_form_per_shift'
        );
        Schema::dropIfExists('cnsd_asmgcs_meter_records');
    }
};
