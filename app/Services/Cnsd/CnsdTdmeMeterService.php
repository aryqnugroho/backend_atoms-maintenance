...
use App\Models\Cnsd\CnsdTdmeMeterItem;
use App\Models\Cnsd\CnsdTdmeMeterRecord;
use App\Models\Cnsd\CnsdTdmeMeterTechnician;
...
class CnsdTdmeMeterService
{
    ...
    {
        $query = CnsdTdmeMeterRecord::query()->withCount('technicians');
    ...
    public function findRecord(int $id): ?CnsdTdmeMeterRecord
    {
        return CnsdTdmeMeterRecord::with(['technicians', 'items', 'manager:id,name', 'supervisor:id,name', 'creator:id,name'])->find($id);
    }

    public function findExistingRecord(string $formType, string $facility, string $date, string $shiftType): ?CnsdTdmeMeterRecord
    {
        return CnsdTdmeMeterRecord::where('form_type', $formType)->where('facility', $facility)->whereDate('date', $date)->where('shift_type', $shiftType)->first();
    }

    public function createRecord(array $data, ?LocalUser $creator = null): CnsdTdmeMeterRecord
    {
        $formType  = $data['form_type']  ?? 'TDME-METER';
        $facility  = $data['facility']   ?? 'T-DME';
        ...
        if ($existing) throw new CnsdTdmeMeterDuplicateException($existing);
        ...
        return DB::transaction(function () use ($formType, $facility, $formCode, $date, $shiftType, $location, $data, $creator, $rosterContext) {
            ...
            $record = CnsdTdmeMeterRecord::create([
            ...
            foreach ($rosterContext['technicians'] as $tech) {
                CnsdTdmeMeterTechnician::create([
                    'tdme_meter_record_id' => $record->id,
            ...
            $itemRows = CnsdTdmeMeterTemplate::buildItemRows($record->id);
            if (!empty($itemRows)) CnsdTdmeMeterItem::insert($itemRows);
    ...
    private function resolveRosterContext(string $shiftType, string $date): array
    {
        ...
        try {
            ...
            Log::warning('CnsdTdmeMeterService: roster lookup failed', ['error' => $e->getMessage()]);
        }

        return ['manager' => $manager, 'supervisor' => $supervisor, 'technicians' => $technicians];
    }

    /** Format: TDME-{YYMMDD}-{SEQ}  e.g. TDME-260519-001 */
    public function generateFormNumber(string $date): string
    {
        $prefix = 'TDME-' . date('ymd', strtotime($date));
        $count  = CnsdTdmeMeterRecord::withTrashed()->where('form_number', 'LIKE', $prefix . '%')->count();
    ...
    public function signRecord(CnsdTdmeMeterRecord $record, string $role, string $base64, LocalUser $signer, ?int $techRowId = null): CnsdTdmeMeterRecord
    {
    ...
    private function signRecordRole(CnsdTdmeMeterRecord $record, string $role, string $base64, LocalUser $signer): void
    {
        $map = [
    ...
    private function signTechnicianRow(CnsdTdmeMeterRecord $record, string $base64, LocalUser $signer, ?int $techRowId): void
    {
    ...
    public function deleteRecord(CnsdTdmeMeterRecord $record): void { $record->delete(); }
}
