...
use App\Models\Cnsd\CnsdVccsFreqMeterItem;
use App\Models\Cnsd\CnsdVccsFreqMeterRecord;
use App\Models\Cnsd\CnsdVccsFreqMeterTechnician;
...
/**
 * CnsdVccsFreqMeterService — business logic for CNSD VCCS Frequentis Meter Reading.
 * Sister service to CnsdVccsMeterService (LES); independent data so both brands
 * can be filled per-shift without colliding.
 */
class CnsdVccsFreqMeterService
{
    ...
    {
        $query = CnsdVccsFreqMeterRecord::query()->withCount('technicians');
    ...
    public function findRecord(int $id): ?CnsdVccsFreqMeterRecord
    {
        return CnsdVccsFreqMeterRecord::with(['technicians', 'items', 'manager:id,name', 'supervisor:id,name', 'creator:id,name'])
            ->find($id);
    }

    public function findExistingRecord(string $formType, string $facility, string $date, string $shiftType): ?CnsdVccsFreqMeterRecord
    {
        return CnsdVccsFreqMeterRecord::where('form_type', $formType)
    ...
    public function createRecord(array $data, ?LocalUser $creator = null): CnsdVccsFreqMeterRecord
    {
        $formType      = $data['form_type']      ?? 'VCCS-FREQ-METER';
        $facility      = $data['facility']       ?? 'VCCS';
        $date          = $data['date'];
        $shiftType     = $data['shift_type'];
        $location      = $data['location']       ?? 'Kantor Cabang Surabaya / Cabang Surabaya';
        $merk          = array_key_exists('merk', $data) ? $data['merk'] : 'FREQUENTIS';
        ...
        if ($existing) {
            throw new CnsdVccsFreqMeterDuplicateException($existing);
        }
        ...
        return DB::transaction(function () use (
            ...
            $record = CnsdVccsFreqMeterRecord::create([
            ...
            foreach ($rosterContext['technicians'] as $tech) {
                CnsdVccsFreqMeterTechnician::create([
                    'vccs_freq_meter_record_id' => $record->id,
            ...
            $itemRows = CnsdVccsFreqMeterTemplate::buildItemRows($record->id);
            if (!empty($itemRows)) {
                CnsdVccsFreqMeterItem::insert($itemRows);
            }
    ...
    private function resolveRosterContext(string $shiftType, string $date): array
    {
        ...
        try {
            ...
            Log::warning('CnsdVccsFreqMeterService: roster lookup failed', [
                'shift_type' => $shiftType,
                'date'       => $date,
                'error'      => $e->getMessage(),
            ]);
    ...
    /**
     * Format: VCCSF-{YYMMDD}-{SEQ}   (distinct from VCCS LES: VCCS-{YYMMDD})
     */
    public function generateFormNumber(string $formType, string $facility, string $date): string
    {
        $dateYymmdd = date('ymd', strtotime($date));
        $prefix = 'VCCSF-' . $dateYymmdd;

        $count = CnsdVccsFreqMeterRecord::withTrashed()
    ...
    public function updateItems(CnsdVccsFreqMeterRecord $record, array $items): CnsdVccsFreqMeterRecord
    {
        return $this->updateRecord($record, ['items' => $items]);
    }

    public function updateRecord(CnsdVccsFreqMeterRecord $record, array $data): CnsdVccsFreqMeterRecord
    {
        ...
        return DB::transaction(function () use ($record, $data) {
            ...
            if (!empty($data['items']) && is_array($data['items'])) {
                ...
                foreach ($data['items'] as $payload) {
                    ...
                    /** @var CnsdVccsFreqMeterItem $item */
    ...
    public function signRecord(
        CnsdVccsFreqMeterRecord $record,
        string $role,
        string $base64Signature,
        LocalUser $signer,
        ?int $technicianRowId = null,
    ): CnsdVccsFreqMeterRecord {
    ...
    public function deleteRecord(CnsdVccsFreqMeterRecord $record): void
    {
        $record->delete();
    }
}
