...
use App\Models\Cnsd\CnsdDvorMeterItem;
use App\Models\Cnsd\CnsdDvorMeterRecord;
use App\Models\Cnsd\CnsdDvorMeterTechnician;
...
class CnsdDvorMeterService
{
    ...
    {
        $query = CnsdDvorMeterRecord::query()->withCount('technicians');
    ...
    public function findRecord(int $id): ?CnsdDvorMeterRecord
    {
        return CnsdDvorMeterRecord::with(['technicians', 'items', 'manager:id,name', 'supervisor:id,name', 'creator:id,name'])->find($id);
    }

    public function findExistingRecord(string $formType, string $facility, string $date, string $shiftType): ?CnsdDvorMeterRecord
    {
        return CnsdDvorMeterRecord::where('form_type', $formType)->where('facility', $facility)->whereDate('date', $date)->where('shift_type', $shiftType)->first();
    }

    public function createRecord(array $data, ?LocalUser $creator = null): CnsdDvorMeterRecord
    {
        $formType  = $data['form_type']  ?? 'DVOR-METER';
        $facility  = $data['facility']   ?? 'DVOR';
        ...
        if ($existing) throw new CnsdDvorMeterDuplicateException($existing);
        ...
        return DB::transaction(function () use ($formType, $facility, $formCode, $date, $shiftType, $location, $data, $creator, $rosterContext) {
            ...
            $record = CnsdDvorMeterRecord::create([
            ...
            foreach ($rosterContext['technicians'] as $tech) {
                CnsdDvorMeterTechnician::create([
                    'dvor_meter_record_id' => $record->id,
            ...
            $itemRows = CnsdDvorMeterTemplate::buildItemRows($record->id);
            if (!empty($itemRows)) CnsdDvorMeterItem::insert($itemRows);
    ...
    private function resolveRosterContext(string $shiftType, string $date): array
    {
        ...
        try {
            ...
            Log::warning('CnsdDvorMeterService: roster lookup failed', ['error' => $e->getMessage()]);
        }

        return ['manager' => $manager, 'supervisor' => $supervisor, 'technicians' => $technicians];
    }

    /** Format: DVOR-{YYMMDD}-{SEQ}  e.g. DVOR-260519-001 */
    public function generateFormNumber(string $date): string
    {
        $prefix = 'DVOR-' . date('ymd', strtotime($date));
        $count  = CnsdDvorMeterRecord::withTrashed()->where('form_number', 'LIKE', $prefix . '%')->count();
    ...
    public function updateRecord(CnsdDvorMeterRecord $record, array $data): CnsdDvorMeterRecord
    {
        ...
        return DB::transaction(function () use ($record, $data) {
            ...
            foreach ($data['items'] ?? [] as $payload) {
                ...
                foreach (['hasil_pemeriksaan', 'keterangan'] as $col) {
    ...
    public function signRecord(CnsdDvorMeterRecord $record, string $role, string $base64, LocalUser $signer, ?int $techRowId = null): CnsdDvorMeterRecord
    {
    ...
    private function signRecordRole(CnsdDvorMeterRecord $record, string $role, string $base64, LocalUser $signer): void
    {
        $map = [
    ...
    private function signTechnicianRow(CnsdDvorMeterRecord $record, string $base64, LocalUser $signer, ?int $techRowId): void
    {
    ...
    public function deleteRecord(CnsdDvorMeterRecord $record): void { $record->delete(); }
}
