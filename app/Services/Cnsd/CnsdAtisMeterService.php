...
use App\Models\Cnsd\CnsdAtisMeterItem;
use App\Models\Cnsd\CnsdAtisMeterRecord;
use App\Models\Cnsd\CnsdAtisMeterTechnician;
...
class CnsdAtisMeterService
{
    ...
    {
        $query = CnsdAtisMeterRecord::query()->withCount('technicians');
    ...
    public function findRecord(int $id): ?CnsdAtisMeterRecord
    {
        return CnsdAtisMeterRecord::with(['technicians', 'items', 'manager:id,name', 'supervisor:id,name', 'creator:id,name'])->find($id);
    }

    public function findExistingRecord(string $formType, string $facility, string $date, string $shiftType): ?CnsdAtisMeterRecord
    {
        return CnsdAtisMeterRecord::where('form_type', $formType)->where('facility', $facility)->whereDate('date', $date)->where('shift_type', $shiftType)->first();
    }

    public function createRecord(array $data, ?LocalUser $creator = null): CnsdAtisMeterRecord
    {
        $formType  = $data['form_type']  ?? 'ATIS-METER';
        $facility  = $data['facility']   ?? 'ATIS';
        ...
        if ($existing) throw new CnsdAtisMeterDuplicateException($existing);
        ...
        return DB::transaction(function () use ($formType, $facility, $formCode, $date, $shiftType, $location, $data, $creator, $rosterContext) {
            ...
            $record = CnsdAtisMeterRecord::create([
                'form_number'     => $this->generateFormNumber($date),
                'form_type'       => $formType,
                'facility'        => $facility,
                'form_code'       => $formCode,
                'merk'            => $data['merk']          ?? 'TERMA',
                'type'            => $data['type']          ?? null,
            ...
            foreach ($rosterContext['technicians'] as $tech) {
                CnsdAtisMeterTechnician::create([
                    'atis_meter_record_id' => $record->id,
            ...
            $itemRows = CnsdAtisMeterTemplate::buildItemRows($record->id);
            if (!empty($itemRows)) CnsdAtisMeterItem::insert($itemRows);
    ...
    private function resolveRosterContext(string $shiftType, string $date): array
    {
        ...
        try {
            ...
            Log::warning('CnsdAtisMeterService: roster lookup failed', ['error' => $e->getMessage()]);
        }

        return ['manager' => $manager, 'supervisor' => $supervisor, 'technicians' => $technicians];
    }

    /** Format: ATIS-{YYMMDD}-{SEQ}  e.g. ATIS-260522-001 */
    public function generateFormNumber(string $date): string
    {
        $prefix = 'ATIS-' . date('ymd', strtotime($date));
        $count  = CnsdAtisMeterRecord::withTrashed()->where('form_number', 'LIKE', $prefix . '%')->count();
        return $prefix . '-' . str_pad($count + 1, 3, '0', STR_PAD_LEFT);
    }

    public function updateItems(CnsdAtisMeterRecord $record, array $items): CnsdAtisMeterRecord
    {
        return $this->updateRecord($record, ['items' => $items]);
    }

    public function updateRecord(CnsdAtisMeterRecord $record, array $data): CnsdAtisMeterRecord
    {
        ...
        return DB::transaction(function () use ($record, $data) {
            ...
            foreach ($items as $payload) {
                ...
                foreach (['reading', 'keterangan'] as $col) {
    ...
    public function signRecord(CnsdAtisMeterRecord $record, string $role, string $base64, LocalUser $signer, ?int $techRowId = null): CnsdAtisMeterRecord
    {
    ...
    private function signRecordRole(CnsdAtisMeterRecord $record, string $role, string $base64, LocalUser $signer): void
    {
        $map = [
    ...
    private function signTechnicianRow(CnsdAtisMeterRecord $record, string $base64, LocalUser $signer, ?int $techRowId): void
    {
    ...
    public function deleteRecord(CnsdAtisMeterRecord $record): void { $record->delete(); }
}
