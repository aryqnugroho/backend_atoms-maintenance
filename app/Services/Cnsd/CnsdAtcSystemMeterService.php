...
use App\Models\Cnsd\CnsdAtcSystemMeterItem;
use App\Models\Cnsd\CnsdAtcSystemMeterRecord;
use App\Models\Cnsd\CnsdAtcSystemMeterTechnician;
...
class CnsdAtcSystemMeterService
{
    ...
    {
        $query = CnsdAtcSystemMeterRecord::query()->withCount('technicians');
    ...
    public function findRecord(int $id): ?CnsdAtcSystemMeterRecord
    {
        return CnsdAtcSystemMeterRecord::with(['technicians', 'items', 'manager:id,name', 'supervisor:id,name', 'creator:id,name'])->find($id);
    }

    public function findExistingRecord(string $formType, string $facility, string $date, string $shiftType): ?CnsdAtcSystemMeterRecord
    {
        return CnsdAtcSystemMeterRecord::where('form_type', $formType)->where('facility', $facility)->whereDate('date', $date)->where('shift_type', $shiftType)->first();
    }

    public function createRecord(array $data, ?LocalUser $creator = null): CnsdAtcSystemMeterRecord
    {
        $formType  = $data['form_type']  ?? 'ATC-SYSTEM-METER';
        $facility  = $data['facility']   ?? 'ATC SYSTEM';
        $formCode  = $data['form_code']  ?? 'FORM A-1';
        $date      = $data['date'];
        $shiftType = $data['shift_type'];
        $location  = $data['location']   ?? 'Cabang Surabaya';

        $existing = $this->findExistingRecord($formType, $facility, $date, $shiftType);
        if ($existing) throw new CnsdAtcSystemMeterDuplicateException($existing);
        ...
        return DB::transaction(function () use ($formType, $facility, $formCode, $date, $shiftType, $location, $data, $creator, $rosterContext) {
            ...
            $record = CnsdAtcSystemMeterRecord::create([
            ...
            foreach ($rosterContext['technicians'] as $tech) {
                CnsdAtcSystemMeterTechnician::create([
                    'atc_system_meter_record_id' => $record->id,
            ...
            $itemRows = CnsdAtcSystemMeterTemplate::buildItemRows($record->id);
            if (!empty($itemRows)) CnsdAtcSystemMeterItem::insert($itemRows);
    ...
    private function resolveRosterContext(string $shiftType, string $date): array
    {
        ...
        try {
            ...
            Log::warning('CnsdAtcSystemMeterService: roster lookup failed', ['error' => $e->getMessage()]);
        }

        return ['manager' => $manager, 'supervisor' => $supervisor, 'technicians' => $technicians];
    }

    /** Format: ATCSYS-{YYMMDD}-{SEQ}  e.g. ATCSYS-260522-001 */
    public function generateFormNumber(string $date): string
    {
        $prefix = 'ATCSYS-' . date('ymd', strtotime($date));
        $count  = CnsdAtcSystemMeterRecord::withTrashed()->where('form_number', 'LIKE', $prefix . '%')->count();
        return $prefix . '-' . str_pad($count + 1, 3, '0', STR_PAD_LEFT);
    }

    public function updateItems(CnsdAtcSystemMeterRecord $record, array $items): CnsdAtcSystemMeterRecord
    {
        return $this->updateRecord($record, ['items' => $items]);
    }

    public function updateRecord(CnsdAtcSystemMeterRecord $record, array $data): CnsdAtcSystemMeterRecord
    {
        ...
        return DB::transaction(function () use ($record, $data) {
            ...
            foreach ($items as $payload) {
                ...
                foreach (['value_1', 'value_2', 'value_3', 'value_4', 'status_flags', 'keterangan'] as $col) {
    ...
    public function signRecord(CnsdAtcSystemMeterRecord $record, string $role, string $base64, LocalUser $signer, ?int $techRowId = null): CnsdAtcSystemMeterRecord
    {
    ...
    private function signRecordRole(CnsdAtcSystemMeterRecord $record, string $role, string $base64, LocalUser $signer): void
    {
        $map = [
    ...
    private function signTechnicianRow(CnsdAtcSystemMeterRecord $record, string $base64, LocalUser $signer, ?int $techRowId): void
    {
    ...
    public function deleteRecord(CnsdAtcSystemMeterRecord $record): void { $record->delete(); }
}
