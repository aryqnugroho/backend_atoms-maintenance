...
use App\Models\Cnsd\CnsdDmeMeterItem;
use App\Models\Cnsd\CnsdDmeMeterRecord;
use App\Models\Cnsd\CnsdDmeMeterTechnician;
...
class CnsdDmeMeterService
{
    ...
    {
        $query = CnsdDmeMeterRecord::query()->withCount('technicians');
    ...
    public function findRecord(int $id): ?CnsdDmeMeterRecord
    {
        return CnsdDmeMeterRecord::with(['technicians', 'items', 'manager:id,name', 'supervisor:id,name', 'creator:id,name'])->find($id);
    }

    public function findExistingRecord(string $formType, string $facility, string $date, string $shiftType): ?CnsdDmeMeterRecord
    {
        return CnsdDmeMeterRecord::where('form_type', $formType)->where('facility', $facility)->whereDate('date', $date)->where('shift_type', $shiftType)->first();
    }

    public function createRecord(array $data, ?LocalUser $creator = null): CnsdDmeMeterRecord
    {
        $formType  = $data['form_type']  ?? 'DME-METER';
        $facility  = $data['facility']   ?? 'DME';
        $formCode  = $data['form_code']  ?? 'FORM N-5';
        $date      = $data['date'];
        $shiftType = $data['shift_type'];
        $location  = $data['location']   ?? 'Surabaya';

        $existing = $this->findExistingRecord($formType, $facility, $date, $shiftType);
        if ($existing) throw new CnsdDmeMeterDuplicateException($existing);
        ...
        return DB::transaction(function () use ($formType, $facility, $formCode, $date, $shiftType, $location, $data, $creator, $rosterContext) {
            ...
            $record = CnsdDmeMeterRecord::create([
            ...
            foreach ($rosterContext['technicians'] as $tech) {
                CnsdDmeMeterTechnician::create([
                    'dme_meter_record_id' => $record->id,
            ...
            $itemRows = CnsdDmeMeterTemplate::buildItemRows($record->id);
            if (!empty($itemRows)) CnsdDmeMeterItem::insert($itemRows);
    ...
    private function resolveRosterContext(string $shiftType, string $date): array
    {
        ...
        try {
            ...
            Log::warning('CnsdDmeMeterService: roster lookup failed', ['error' => $e->getMessage()]);
        }

        return ['manager' => $manager, 'supervisor' => $supervisor, 'technicians' => $technicians];
    }

    /** Format: DME-{YYMMDD}-{SEQ}  e.g. DME-260522-001 */
    public function generateFormNumber(string $date): string
    {
        $prefix = 'DME-' . date('ymd', strtotime($date));
        $count  = CnsdDmeMeterRecord::withTrashed()->where('form_number', 'LIKE', $prefix . '%')->count();
        return $prefix . '-' . str_pad($count + 1, 3, '0', STR_PAD_LEFT);
    }

    public function updateItems(CnsdDmeMeterRecord $record, array $items): CnsdDmeMeterRecord
    {
        return $this->updateRecord($record, ['items' => $items]);
    }

    public function updateRecord(CnsdDmeMeterRecord $record, array $data): CnsdDmeMeterRecord
    {
    ...
    public function signRecord(CnsdDmeMeterRecord $record, string $role, string $base64, LocalUser $signer, ?int $techRowId = null): CnsdDmeMeterRecord
    {
    ...
    private function signRecordRole(CnsdDmeMeterRecord $record, string $role, string $base64, LocalUser $signer): void
    {
        $map = [
    ...
    private function signTechnicianRow(CnsdDmeMeterRecord $record, string $base64, LocalUser $signer, ?int $techRowId): void
    {
    ...
    public function deleteRecord(CnsdDmeMeterRecord $record): void { $record->delete(); }
}
