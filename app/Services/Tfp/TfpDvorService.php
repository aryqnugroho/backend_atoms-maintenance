...
class TfpDvorService
{
    ...
    {
        return TfpDvorRecord::query()->where('form_type', $formType)->whereDate('date', $date)->where('shift_type', $shiftType)->first();
    }
    ...
    public function createRecord(array $data, ?LocalUser $creator = null): TfpDvorRecord
    {
        ...
        return DB::transaction(function () use ($formType, $date, $shiftType, $location, $creator, $rosterContext) {
            ...
            foreach ($rosterContext['technicians'] as $tech) {
                TfpDvorTechnician::create(['dvor_record_id' => $record->id, 'technician_id' => $tech['local_id'], 'technician_name' => $tech['name'], 'sort_order' => $sort++]);
            }
    ...
    public function updateItems(TfpDvorRecord $record, array $items): TfpDvorRecord
    {
        ...
        return DB::transaction(function () use ($record, $items) {
            ...
            $allowed  = ['panel_d01', 'panel_d03', 'panel_d04', 'panel_d05', 'panel_ats_d06_input', 'panel_ats_d06_output'];
    ...
    public function updateFacilities(TfpDvorRecord $record, array $facilities): TfpDvorRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah lagi.');
