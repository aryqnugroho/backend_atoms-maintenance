...
use App\Models\Tfp\TfpDvorFacility;
use App\Models\Tfp\TfpDvorItem;
use App\Models\Tfp\TfpDvorRecord;
use App\Models\Tfp\TfpDvorTechnician;
...
class TfpDvorService
{
    ...
    {
        $query = TfpDvorRecord::query()
            ->with(['technicians:id,dvor_record_id,technician_id,technician_name,technician_signature,sort_order', 'manager:id,name', 'supervisor:id,name'])
            ->withCount('technicians');

        $query->byFormType($filters['form_type'] ?? 'DVOR');
    ...
    public function findRecord(int $id): ?TfpDvorRecord
    {
        return TfpDvorRecord::query()
            ->with(['technicians', 'items', 'facilities', 'manager:id,name', 'supervisor:id,name', 'creator:id,name'])
            ->find($id);
    }

    public function findExistingRecord(string $formType, string $date, string $shiftType): ?TfpDvorRecord
    {
        return TfpDvorRecord::query()
    ...
    public function createRecord(array $data, ?LocalUser $creator = null): TfpDvorRecord
    {
        $formType  = $data['form_type']  ?? 'DVOR';
        $date      = $data['date'];
        $shiftType = $data['shift_type'];
        $location  = $data['location']   ?? 'GEDUNG DVOR';

        $existing = $this->findExistingRecord($formType, $date, $shiftType);
        if ($existing) throw new TfpDvorDuplicateException($existing);
        ...
        return DB::transaction(function () use ($formType, $date, $shiftType, $location, $creator, $rosterContext) {
            ...
            $record = TfpDvorRecord::create([
                ...
                'location'        => $location,
                'columns_config'  => TfpDvorTemplate::defaultColumnsConfig(),
                'status'          => 'ongoing',
            ...
            foreach ($rosterContext['technicians'] as $tech) {
                TfpDvorTechnician::create([
                    'dvor_record_id'  => $record->id,
            ...
            $itemRows = TfpDvorTemplate::buildItemRows($record->id);
            if (!empty($itemRows)) TfpDvorItem::insert($itemRows);

            $facilityRows = TfpDvorTemplate::buildFacilityRows($record->id);
            if (!empty($facilityRows)) TfpDvorFacility::insert($facilityRows);
    ...
    private function resolveRosterContext(string $shiftType, string $date): array
    {
        ...
        try {
            ...
            Log::warning('TfpDvorService: roster lookup failed', ['shift_type' => $shiftType, 'date' => $date, 'error' => $e->getMessage()]);
        }
    ...
    public function generateFormNumber(string $date): string
    {
        ...
        $prefix = 'TFP-DVOR-' . $dateYymmdd;
        $count  = TfpDvorRecord::withTrashed()->where('form_number', 'LIKE', $prefix . '%')->count();
        return $prefix . '-' . str_pad($count + 1, 3, '0', STR_PAD_LEFT);
    }

    // ─── Update items + facilities ─────────────────────────────

    public function updateItems(TfpDvorRecord $record, array $items, ?string $timeOverride = null): TfpDvorRecord
    {
    ...
    public function updateFacilities(TfpDvorRecord $record, array $facilities, ?string $timeOverride = null): TfpDvorRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah lagi.');
    ...
    private function cellKeysFromConfig(TfpDvorRecord $record): array
    {
    ...
    public function addParameter(TfpDvorRecord $record, array $data): TfpDvorRecord
    {
    ...
    public function updateParameterStructure(TfpDvorRecord $record, int $paramId, array $data): TfpDvorRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');

        /** @var TfpDvorItem|null $item */
    ...
    public function deleteParameter(TfpDvorRecord $record, int $paramId): TfpDvorRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
    ...
    public function reorderParameters(TfpDvorRecord $record, array $orderedIds): TfpDvorRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
    ...
    public function addFacility(TfpDvorRecord $record, array $data): TfpDvorRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
        $maxSort = (int) ($record->facilities()->max('sort_order') ?? -1);
        TfpDvorFacility::create([
            'dvor_record_id'  => $record->id,
    ...
    public function updateFacilityStructure(TfpDvorRecord $record, int $facilityId, array $data): TfpDvorRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
        /** @var TfpDvorFacility|null $facility */
    ...
    public function deleteFacility(TfpDvorRecord $record, int $facilityId): TfpDvorRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
    ...
    public function reorderFacilities(TfpDvorRecord $record, array $orderedIds): TfpDvorRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
        DB::transaction(function () use ($record, $orderedIds) {
            foreach ($orderedIds as $index => $id) {
    ...
    private function fresh(TfpDvorRecord $record): TfpDvorRecord
    {
        return $record->fresh(['technicians', 'items', 'facilities', 'manager:id,name', 'supervisor:id,name']);
    }

    public function saveStructure(TfpDvorRecord $record, array $columnsConfig, array $itemPatches): TfpDvorRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
    ...
    public function signRecord(TfpDvorRecord $record, string $role, string $base64Signature, LocalUser $signer, ?int $technicianRowId = null): TfpDvorRecord
    {
    ...
    private function signRecordRole(TfpDvorRecord $record, string $role, string $base64, LocalUser $signer): void
    {
    ...
    private function signTechnicianRow(TfpDvorRecord $record, string $base64, LocalUser $signer, ?int $technicianRowId): void
    {
    ...
    public function deleteRecord(TfpDvorRecord $record): void { $record->delete(); }
}
