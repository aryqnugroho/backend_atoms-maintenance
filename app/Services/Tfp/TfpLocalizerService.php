...
class TfpLocalizerService
{
    ...
    {
        return TfpLocalizerRecord::query()->where('form_type', $formType)->whereDate('date', $date)->where('shift_type', $shiftType)->first();
    }
    ...
    public function createRecord(array $data, ?LocalUser $creator = null): TfpLocalizerRecord
    {
        ...
        return DB::transaction(function () use ($formType, $date, $shiftType, $location, $creator, $rosterContext) {
            ...
            foreach ($rosterContext['technicians'] as $tech) {
                TfpLocalizerTechnician::create(['localizer_record_id' => $record->id, 'technician_id' => $tech['local_id'], 'technician_name' => $tech['name'], 'sort_order' => $sort++]);
            }
    ...
    public function generateFormNumber(string $date): string
    {
        ...
        $prefix = 'TFP-LZ-' . $dateYymmdd;
        $count  = TfpLocalizerRecord::withTrashed()->where('form_number', 'LIKE', $prefix . '%')->count();
        return $prefix . '-' . str_pad($count + 1, 3, '0', STR_PAD_LEFT);
    }

    public function updateItems(TfpLocalizerRecord $record, array $items): TfpLocalizerRecord
    {
        ...
        return DB::transaction(function () use ($record, $items) {
            ...
            $allowed  = ['panel_lz01', 'panel_cos_lz02_input', 'panel_cos_lz02_output', 'panel_lz03', 'panel_cos_lz04_input', 'panel_cos_lz04_output', 'panel_mlat_ru04'];
    ...
    public function updateFacilities(TfpLocalizerRecord $record, array $facilities): TfpLocalizerRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah lagi.');
    ...
    private function signRecordRole(TfpLocalizerRecord $record, string $role, string $base64, LocalUser $signer): void
    {
        ...
        $roleOk = match ($role) { 'manager' => $signer->isManager(), 'supervisor' => $signer->isSupervisorTfp() || $signer->isSupervisor(), default => false };
