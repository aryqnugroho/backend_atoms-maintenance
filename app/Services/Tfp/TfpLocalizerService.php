...
use App\Exceptions\TfpLocalizerDuplicateException;
use App\Models\LocalUser;
use App\Models\Tfp\TfpLocalizerFacility;
use App\Models\Tfp\TfpLocalizerItem;
use App\Models\Tfp\TfpLocalizerRecord;
use App\Models\Tfp\TfpLocalizerTechnician;
...
class TfpLocalizerService
{
    ...
    {
        $query = TfpLocalizerRecord::query()
            ->with(['technicians:id,localizer_record_id,technician_id,technician_name,technician_signature,sort_order', 'manager:id,name', 'supervisor:id,name'])
            ->withCount('technicians');

        $query->byFormType($filters['form_type'] ?? 'LOCALIZER');
    ...
    public function findRecord(int $id): ?TfpLocalizerRecord
    {
        return TfpLocalizerRecord::query()
            ->with(['technicians', 'items', 'facilities', 'manager:id,name', 'supervisor:id,name', 'creator:id,name'])
            ->find($id);
    }

    public function findExistingRecord(string $formType, string $date, string $shiftType): ?TfpLocalizerRecord
    {
        return TfpLocalizerRecord::query()
    ...
    public function createRecord(array $data, ?LocalUser $creator = null): TfpLocalizerRecord
    {
        $formType  = $data['form_type']  ?? 'LOCALIZER';
        $date      = $data['date'];
        $shiftType = $data['shift_type'];
        $location  = $data['location']   ?? 'GEDUNG LOCALIZER';

        $existing = $this->findExistingRecord($formType, $date, $shiftType);
        if ($existing) throw new TfpLocalizerDuplicateException($existing);
        ...
        return DB::transaction(function () use ($formType, $date, $shiftType, $location, $creator, $rosterContext) {
            ...
            $record = TfpLocalizerRecord::create([
                ...
                'location'        => $location,
                'columns_config'  => TfpLocalizerTemplate::defaultColumnsConfig(),
                'status'          => 'ongoing',
            ...
            foreach ($rosterContext['technicians'] as $tech) {
                TfpLocalizerTechnician::create([
                    'localizer_record_id' => $record->id,
            ...
            $itemRows = TfpLocalizerTemplate::buildItemRows($record->id);
            if (!empty($itemRows)) TfpLocalizerItem::insert($itemRows);

            $facilityRows = TfpLocalizerTemplate::buildFacilityRows($record->id);
            if (!empty($facilityRows)) TfpLocalizerFacility::insert($facilityRows);
    ...
    private function resolveRosterContext(string $shiftType, string $date): array
    {
        ...
        try {
            ...
            Log::warning('TfpLocalizerService: roster lookup failed', ['shift_type' => $shiftType, 'date' => $date, 'error' => $e->getMessage()]);
        }
    ...
    public function generateFormNumber(string $date): string
    {
        ...
        $prefix = 'TFP-LLZ-' . $dateYymmdd;
        $count  = TfpLocalizerRecord::withTrashed()->where('form_number', 'LIKE', $prefix . '%')->count();
        return $prefix . '-' . str_pad($count + 1, 3, '0', STR_PAD_LEFT);
    }

    // ─── Update items + facilities ─────────────────────────────

    public function updateItems(TfpLocalizerRecord $record, array $items, ?string $timeOverride = null): TfpLocalizerRecord
    {
    ...
    public function updateFacilities(TfpLocalizerRecord $record, array $facilities, ?string $timeOverride = null): TfpLocalizerRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah lagi.');
    ...
    private function cellKeysFromConfig(TfpLocalizerRecord $record): array
    {
    ...
    public function addParameter(TfpLocalizerRecord $record, array $data): TfpLocalizerRecord
    {
    ...
    public function updateParameterStructure(TfpLocalizerRecord $record, int $paramId, array $data): TfpLocalizerRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');

        /** @var TfpLocalizerItem|null $item */
    ...
    public function deleteParameter(TfpLocalizerRecord $record, int $paramId): TfpLocalizerRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
    ...
    public function reorderParameters(TfpLocalizerRecord $record, array $orderedIds): TfpLocalizerRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
    ...
    public function addFacility(TfpLocalizerRecord $record, array $data): TfpLocalizerRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
        $maxSort = (int) ($record->facilities()->max('sort_order') ?? -1);
        TfpLocalizerFacility::create([
            'localizer_record_id' => $record->id,
    ...
    public function updateFacilityStructure(TfpLocalizerRecord $record, int $facilityId, array $data): TfpLocalizerRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
        /** @var TfpLocalizerFacility|null $facility */
    ...
    public function deleteFacility(TfpLocalizerRecord $record, int $facilityId): TfpLocalizerRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
    ...
    public function reorderFacilities(TfpLocalizerRecord $record, array $orderedIds): TfpLocalizerRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
        DB::transaction(function () use ($record, $orderedIds) {
            foreach ($orderedIds as $index => $id) {
    ...
    private function fresh(TfpLocalizerRecord $record): TfpLocalizerRecord
    {
        return $record->fresh(['technicians', 'items', 'facilities', 'manager:id,name', 'supervisor:id,name']);
    }

    public function saveStructure(TfpLocalizerRecord $record, array $columnsConfig, array $itemPatches): TfpLocalizerRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
    ...
    public function signRecord(TfpLocalizerRecord $record, string $role, string $base64Signature, LocalUser $signer, ?int $technicianRowId = null): TfpLocalizerRecord
    {
    ...
    private function signRecordRole(TfpLocalizerRecord $record, string $role, string $base64, LocalUser $signer): void
    {
    ...
    private function signTechnicianRow(TfpLocalizerRecord $record, string $base64, LocalUser $signer, ?int $technicianRowId): void
    {
    ...
    public function deleteRecord(TfpLocalizerRecord $record): void { $record->delete(); }
}
