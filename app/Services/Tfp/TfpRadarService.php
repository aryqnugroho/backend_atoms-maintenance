...
use App\Exceptions\TfpRadarDuplicateException;
use App\Models\LocalUser;
use App\Models\Tfp\TfpRadarFacility;
use App\Models\Tfp\TfpRadarItem;
use App\Models\Tfp\TfpRadarRecord;
use App\Models\Tfp\TfpRadarTechnician;
...
class TfpRadarService
{
    ...
    {
        $query = TfpRadarRecord::query()
            ->with(['technicians:id,radar_record_id,technician_id,technician_name,technician_signature,sort_order', 'manager:id,name', 'supervisor:id,name'])
            ->withCount('technicians');

        $query->byFormType($filters['form_type'] ?? 'RADAR');
    ...
    public function findRecord(int $id): ?TfpRadarRecord
    {
        return TfpRadarRecord::query()
            ->with(['technicians', 'items', 'facilities', 'manager:id,name', 'supervisor:id,name', 'creator:id,name'])
            ->find($id);
    }

    public function findExistingRecord(string $formType, string $date, string $shiftType): ?TfpRadarRecord
    {
        return TfpRadarRecord::query()
    ...
    public function createRecord(array $data, ?LocalUser $creator = null): TfpRadarRecord
    {
        $formType  = $data['form_type']  ?? 'RADAR';
        $date      = $data['date'];
        $shiftType = $data['shift_type'];
        $location  = $data['location']   ?? 'GEDUNG RADAR';

        $existing = $this->findExistingRecord($formType, $date, $shiftType);
        if ($existing) throw new TfpRadarDuplicateException($existing);
        ...
        return DB::transaction(function () use ($formType, $date, $shiftType, $location, $creator, $rosterContext) {
            ...
            $record = TfpRadarRecord::create([
                ...
                'location'        => $location,
                'columns_config'  => TfpRadarTemplate::defaultColumnsConfig(),
                'status'          => 'ongoing',
            ...
            foreach ($rosterContext['technicians'] as $tech) {
                TfpRadarTechnician::create([
                    'radar_record_id' => $record->id,
            ...
            $itemRows = TfpRadarTemplate::buildItemRows($record->id);
            if (!empty($itemRows)) TfpRadarItem::insert($itemRows);

            $facilityRows = TfpRadarTemplate::buildFacilityRows($record->id);
            if (!empty($facilityRows)) TfpRadarFacility::insert($facilityRows);
    ...
    private function resolveRosterContext(string $shiftType, string $date): array
    {
        ...
        try {
            ...
            Log::warning('TfpRadarService: roster lookup failed', ['shift_type' => $shiftType, 'date' => $date, 'error' => $e->getMessage()]);
        }
    ...
    public function generateFormNumber(string $date): string
    {
        ...
        $prefix = 'TFP-RADAR-' . $dateYymmdd;
        $count  = TfpRadarRecord::withTrashed()->where('form_number', 'LIKE', $prefix . '%')->count();
        return $prefix . '-' . str_pad($count + 1, 3, '0', STR_PAD_LEFT);
    }

    // ─── Update items + facilities ─────────────────────────────

    public function updateItems(TfpRadarRecord $record, array $items, ?string $timeOverride = null): TfpRadarRecord
    {
    ...
    public function updateFacilities(TfpRadarRecord $record, array $facilities, ?string $timeOverride = null): TfpRadarRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah lagi.');
    ...
    private function cellKeysFromConfig(TfpRadarRecord $record): array
    {
    ...
    public function addParameter(TfpRadarRecord $record, array $data): TfpRadarRecord
    {
    ...
    public function updateParameterStructure(TfpRadarRecord $record, int $paramId, array $data): TfpRadarRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');

        /** @var TfpRadarItem|null $item */
    ...
    public function deleteParameter(TfpRadarRecord $record, int $paramId): TfpRadarRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
    ...
    public function reorderParameters(TfpRadarRecord $record, array $orderedIds): TfpRadarRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
    ...
    public function addFacility(TfpRadarRecord $record, array $data): TfpRadarRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
        $maxSort = (int) ($record->facilities()->max('sort_order') ?? -1);
        TfpRadarFacility::create([
            'radar_record_id' => $record->id,
    ...
    public function updateFacilityStructure(TfpRadarRecord $record, int $facilityId, array $data): TfpRadarRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
        /** @var TfpRadarFacility|null $facility */
    ...
    public function deleteFacility(TfpRadarRecord $record, int $facilityId): TfpRadarRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
    ...
    public function reorderFacilities(TfpRadarRecord $record, array $orderedIds): TfpRadarRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
        DB::transaction(function () use ($record, $orderedIds) {
            foreach ($orderedIds as $index => $id) {
    ...
    private function fresh(TfpRadarRecord $record): TfpRadarRecord
    {
        return $record->fresh(['technicians', 'items', 'facilities', 'manager:id,name', 'supervisor:id,name']);
    }

    public function saveStructure(TfpRadarRecord $record, array $columnsConfig, array $itemPatches): TfpRadarRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah strukturnya.');
    ...
    public function signRecord(TfpRadarRecord $record, string $role, string $base64Signature, LocalUser $signer, ?int $technicianRowId = null): TfpRadarRecord
    {
    ...
    private function signRecordRole(TfpRadarRecord $record, string $role, string $base64, LocalUser $signer): void
    {
    ...
    private function signTechnicianRow(TfpRadarRecord $record, string $base64, LocalUser $signer, ?int $technicianRowId): void
    {
    ...
    public function deleteRecord(TfpRadarRecord $record): void { $record->delete(); }
}
