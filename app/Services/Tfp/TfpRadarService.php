...
class TfpRadarService
{
    ...
    public function listRecords(array $filters, int $perPage = 15): LengthAwarePaginator
    {
        ...
        $query->byFormType($filters['form_type'] ?? 'RADAR-TFP');
    ...
    {
        return TfpRadarRecord::query()->where('form_type', $formType)->whereDate('date', $date)->where('shift_type', $shiftType)->first();
    }

    public function createRecord(array $data, ?LocalUser $creator = null): TfpRadarRecord
    {
        $formType  = $data['form_type']  ?? 'RADAR-TFP';
        $date      = $data['date'];
        ...
        return DB::transaction(function () use ($formType, $date, $shiftType, $location, $creator, $rosterContext) {
            ...
            foreach ($rosterContext['technicians'] as $tech) {
                TfpRadarTechnician::create(['radar_record_id' => $record->id, 'technician_id' => $tech['local_id'], 'technician_name' => $tech['name'], 'sort_order' => $sort++]);
            }
    ...
    public function updateItems(TfpRadarRecord $record, array $items): TfpRadarRecord
    {
        ...
        return DB::transaction(function () use ($record, $items) {
            ...
            $allowed  = ['panel_rd01', 'panel_rd02', 'panel_cos_rd03_input', 'panel_cos_rd03_output', 'ups_topaz_input', 'ups_topaz_output', 'panel_rd04', 'panel_rd05', 'panel_rd06', 'panel_rd07', 'panel_rd08'];
    ...
    public function updateFacilities(TfpRadarRecord $record, array $facilities): TfpRadarRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah lagi.');
