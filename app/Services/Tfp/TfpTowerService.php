...
class TfpTowerService
{
    ...
    {
        return TfpTowerRecord::query()->where('form_type', $formType)->whereDate('date', $date)->where('shift_type', $shiftType)->first();
    }
    ...
    public function createRecord(array $data, ?LocalUser $creator = null): TfpTowerRecord
    {
        ...
        return DB::transaction(function () use ($formType, $date, $shiftType, $location, $creator, $rosterContext) {
            ...
            foreach ($rosterContext['technicians'] as $tech) {
                TfpTowerTechnician::create(['tower_record_id' => $record->id, 'technician_id' => $tech['local_id'], 'technician_name' => $tech['name'], 'sort_order' => $sort++]);
            }
    ...
    public function updateItems(TfpTowerRecord $record, array $items): TfpTowerRecord
    {
        ...
        return DB::transaction(function () use ($record, $items) {
            ...
            $allowed  = ['panel_a10', 'panel_a11', 'panel_ats_a13_input', 'panel_ats_a13_output', 'panel_a14', 'panel_a16', 'panel_a17', 'panel_a18', 'panel_a19', 'panel_a20', 'panel_milat_ru1213'];
    ...
    public function updateFacilities(TfpTowerRecord $record, array $facilities): TfpTowerRecord
    {
        if ($record->status === 'completed') throw new RuntimeException('Form yang sudah completed tidak dapat diubah lagi.');
