...
use App\Http\Requests\Cnsd\CreateCnsdAsmgcsMeterRequest;
use App\Http\Requests\Cnsd\SignCnsdAsmgcsMeterRequest;
use App\Http\Requests\Cnsd\UpdateCnsdAsmgcsMeterRequest;
use App\Models\Cnsd\CnsdAsmgcsMeterRecord;
use App\Services\Cnsd\CnsdActivityLogger;
use App\Services\Cnsd\CnsdAsmgcsMeterService;
use App\Services\Cnsd\CnsdAsmgcsMeterTemplate;
...
class CnsdAsmgcsMeterController extends Controller
{
    use ApiResponse;

    public function __construct(
        protected CnsdAsmgcsMeterService $service,
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $records->through(fn (CnsdAsmgcsMeterRecord $r) => $this->summarizeRecord($r));

        return $this->success($records, 'CNSD ASMGCS Meter records retrieved successfully');
    }

    public function template(): JsonResponse
    {
        return $this->success([
            'form_type'             => 'ASMGCS-METER',
            'facility'              => 'ASMGCS',
            'merk_default'          => 'SAAB',
            'type_default'          => null,
            'serial_number_default' => null,
            'sections'              => CnsdAsmgcsMeterTemplate::sections(),
        ], 'ASMGCS Meter template retrieved successfully');
    }

    public function store(CreateCnsdAsmgcsMeterRequest $request): JsonResponse
    {
        ...
        } catch (CnsdAsmgcsMeterDuplicateException $e) {
            return $this->error(
                $e->getMessage(),
        ...
        try {
            $this->activityLogger->logMeterReadingCreated($record, 'ASMGCS', '/cnsd/asmgcs-meter', $user);
        } catch (\Throwable) { /* non-fatal */ }

        return $this->success($this->detailRecord($record), 'CNSD ASMGCS Meter record created successfully', 201);
    }
    ...
    public function show(int $id): JsonResponse
    {
        ...
        return $this->success($this->detailRecord($record), 'CNSD ASMGCS Meter record retrieved successfully');
    }

    public function update(UpdateCnsdAsmgcsMeterRequest $request, int $id): JsonResponse
    {
        $record = CnsdAsmgcsMeterRecord::find($id);
        if (!$record) {
            return $this->error('Form tidak ditemukan.', null, 404);
        }
        ...
        return $this->success($this->detailRecord($record), 'CNSD ASMGCS Meter record updated successfully');
    }

    public function sign(SignCnsdAsmgcsMeterRequest $request, int $id): JsonResponse
    {
        $record = CnsdAsmgcsMeterRecord::find($id);
        if (!$record) {
            return $this->error('Form tidak ditemukan.', null, 404);
    ...
    public function destroy(int $id): JsonResponse
    {
        ...
        return $this->success(null, 'CNSD ASMGCS Meter record deleted successfully');
    }

    public function years(): JsonResponse
    {
        $years = CnsdAsmgcsMeterRecord::selectRaw('EXTRACT(YEAR FROM date)::int AS y')
            ->whereNotNull('date')
            ->groupBy('y')
            ->orderByDesc('y')
            ->pluck('y')
    ...
    private function summarizeRecord(CnsdAsmgcsMeterRecord $r): array
    {
        return [
            'id'                => $r->id,
    ...
    private function detailRecord(CnsdAsmgcsMeterRecord $r): array
    {
        ...
        return [
            ...
            'sections_meta' => CnsdAsmgcsMeterTemplate::sectionMeta(),
