...
use App\Exceptions\CnsdVccsFreqMeterDuplicateException;
use App\Exceptions\SignerNotAuthorizedException;
use App\Http\Controllers\Controller;
use App\Http\Requests\Cnsd\CreateCnsdVccsFreqMeterRequest;
use App\Http\Requests\Cnsd\SignCnsdVccsFreqMeterRequest;
use App\Http\Requests\Cnsd\UpdateCnsdVccsFreqMeterRequest;
use App\Models\Cnsd\CnsdVccsFreqMeterRecord;
use App\Services\Cnsd\CnsdActivityLogger;
use App\Services\Cnsd\CnsdVccsFreqMeterService;
use App\Services\Cnsd\CnsdVccsFreqMeterTemplate;
...
class CnsdVccsFreqMeterController extends Controller
{
    use ApiResponse;

    public function __construct(
        protected CnsdVccsFreqMeterService $service,
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $records->through(fn (CnsdVccsFreqMeterRecord $r) => $this->summarizeRecord($r));

        return $this->success($records, 'CNSD VCCS Frequentis Meter records retrieved successfully');
    }

    public function template(): JsonResponse
    {
        return $this->success([
            'form_type'             => 'VCCS-FREQ-METER',
            'facility'              => 'VCCS',
            'merk_default'          => 'FREQUENTIS',
            'type_default'          => null,
            'serial_number_default' => null,
            'sections'              => CnsdVccsFreqMeterTemplate::sections(),
        ], 'VCCS Frequentis Meter template retrieved successfully');
    }

    public function store(CreateCnsdVccsFreqMeterRequest $request): JsonResponse
    {
        ...
        } catch (CnsdVccsFreqMeterDuplicateException $e) {
            return $this->error(
                $e->getMessage(),
        ...
        try {
            $this->activityLogger->logMeterReadingCreated($record, 'VCCS Frequentis', '/cnsd/vccs-freq-meter', $user);
        } catch (\Throwable) { /* non-fatal */ }

        return $this->success($this->detailRecord($record), 'CNSD VCCS Frequentis Meter record created successfully', 201);
    }
    ...
    public function show(int $id): JsonResponse
    {
        ...
        return $this->success($this->detailRecord($record), 'CNSD VCCS Frequentis Meter record retrieved successfully');
    }

    public function update(UpdateCnsdVccsFreqMeterRequest $request, int $id): JsonResponse
    {
        $record = CnsdVccsFreqMeterRecord::find($id);
        if (!$record) {
            return $this->error('Form tidak ditemukan.', null, 404);
        }
        ...
        return $this->success($this->detailRecord($record), 'CNSD VCCS Frequentis Meter record updated successfully');
    }

    public function sign(SignCnsdVccsFreqMeterRequest $request, int $id): JsonResponse
    {
        $record = CnsdVccsFreqMeterRecord::find($id);
        if (!$record) {
            return $this->error('Form tidak ditemukan.', null, 404);
    ...
    public function destroy(int $id): JsonResponse
    {
        ...
        return $this->success(null, 'CNSD VCCS Frequentis Meter record deleted successfully');
    }

    public function years(): JsonResponse
    {
        $years = CnsdVccsFreqMeterRecord::selectRaw('EXTRACT(YEAR FROM date)::int AS y')
            ->whereNotNull('date')
            ->groupBy('y')
            ->orderByDesc('y')
            ->pluck('y')
    ...
    private function summarizeRecord(CnsdVccsFreqMeterRecord $r): array
    {
        return [
            'id'                => $r->id,
    ...
    private function detailRecord(CnsdVccsFreqMeterRecord $r): array
    {
        ...
        return [
            ...
            'sections_meta' => CnsdVccsFreqMeterTemplate::sectionMeta(),
