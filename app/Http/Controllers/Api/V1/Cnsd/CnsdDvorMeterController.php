...
use App\Exceptions\CnsdDvorMeterDuplicateException;
use App\Exceptions\SignerNotAuthorizedException;
use App\Http\Controllers\Controller;
use App\Http\Requests\Cnsd\CreateCnsdDvorMeterRequest;
...
class CnsdDvorMeterController extends Controller
{
    use ApiResponse;
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $records->through(fn (CnsdDvorMeterRecord $r) => $this->summarize($r));
        return $this->success($records, 'CNSD DVOR Meter records retrieved successfully');
    }

    /** GET /api/v1/cnsd/dvor-meter/template */
    public function template(): JsonResponse
    {
        return $this->success([
            'form_type' => 'DVOR-METER',
            'facility'  => 'DVOR',
            'form_code' => 'FORM N-5',
            'sections'  => CnsdDvorMeterTemplate::sections(),
        ], 'DVOR Meter template retrieved successfully');
    }

    /** GET /api/v1/cnsd/dvor-meter/years */
    public function years(): JsonResponse
    {
        $years = CnsdDvorMeterRecord::selectRaw('EXTRACT(YEAR FROM date)::int AS y')
    ...
    /** POST /api/v1/cnsd/dvor-meter */
    public function store(CreateCnsdDvorMeterRequest $request): JsonResponse
    {
        $user = Auth::user();
        ...
        } catch (CnsdDvorMeterDuplicateException $e) {
    ...
    /** GET /api/v1/cnsd/dvor-meter/{id} */
    public function show(int $id): JsonResponse
    {
        $record = $this->service->findRecord($id);
        if (!$record) return $this->error('Form tidak ditemukan.', null, 404);
        return $this->success($this->detail($record), 'CNSD DVOR Meter record retrieved successfully');
    }

    /** PUT /api/v1/cnsd/dvor-meter/{id} */
    public function update(UpdateCnsdDvorMeterRequest $request, int $id): JsonResponse
    {
        $record = CnsdDvorMeterRecord::find($id);
        if (!$record) return $this->error('Form tidak ditemukan.', null, 404);
        $user = Auth::user();
        if (!$user || (!$user->isAdmin() && !$user->isManager() && !$user->isSupervisor() && !$user->isTeknisi())) {
            return $this->error('Unauthorized.', null, 403);
        ...
        return $this->success($this->detail($record), 'CNSD DVOR Meter record updated successfully');
    }

    /** POST /api/v1/cnsd/dvor-meter/{id}/sign */
    public function sign(SignCnsdDvorMeterRequest $request, int $id): JsonResponse
    {
        $record = CnsdDvorMeterRecord::find($id);
        if (!$record) return $this->error('Form tidak ditemukan.', null, 404);
        $user = Auth::user();
    ...
    /** DELETE /api/v1/cnsd/dvor-meter/{id} */
    public function destroy(int $id): JsonResponse
    {
        ...
        return $this->success(null, 'CNSD DVOR Meter record deleted successfully');
    }

    // ─── Transformers ─────────────────────────────────────────

    private function summarize(CnsdDvorMeterRecord $r): array
    {
        return [
    ...
    private function detail(CnsdDvorMeterRecord $r): array
    {
        ...
        return [
            ...
            'items' => $r->items->map(fn ($it) => [
                ...
                'item_name' => $it->item_name, 'limit_value' => $it->limit_value,
                'hasil_pemeriksaan' => $it->hasil_pemeriksaan,
                'keterangan' => $it->keterangan, 'is_header' => (bool) $it->is_header,
                'sort_order' => $it->sort_order,
            ])->values()->toArray(),
            'sections_meta' => CnsdDvorMeterTemplate::sectionMeta(),
