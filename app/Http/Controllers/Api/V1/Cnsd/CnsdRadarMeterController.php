...
use App\Http\Requests\Cnsd\CreateCnsdRadarMeterRequest;
...
class CnsdRadarMeterController extends Controller
{
    use ApiResponse;
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $records->through(fn (CnsdRadarMeterRecord $r) => $this->summarizeRecord($r));

        return $this->success($records, 'CNSD Radar Meter records retrieved successfully');
    }

    /**
     * GET /api/v1/cnsd/radar-meter/template
     */
    ...
    /**
     * POST /api/v1/cnsd/radar-meter
     */
    public function store(CreateCnsdRadarMeterRequest $request): JsonResponse
    {
        ...
        } catch (CnsdRadarMeterDuplicateException $e) {
            return $this->error(
                $e->getMessage(),
    ...
    /**
     * GET /api/v1/cnsd/radar-meter/{id}
     */
    public function show(int $id): JsonResponse
    {
        ...
        return $this->success($this->detailRecord($record), 'CNSD Radar Meter record retrieved successfully');
    }

    /**
     * PUT /api/v1/cnsd/radar-meter/{id}
     */
    public function update(UpdateCnsdRadarMeterRequest $request, int $id): JsonResponse
    {
        $record = CnsdRadarMeterRecord::find($id);
        if (!$record) {
            return $this->error('Form tidak ditemukan.', null, 404);
        }
        ...
        return $this->success($this->detailRecord($record), 'CNSD Radar Meter record updated successfully');
    }

    /**
     * POST /api/v1/cnsd/radar-meter/{id}/sign
     */
    public function sign(SignCnsdRadarMeterRequest $request, int $id): JsonResponse
    {
        $record = CnsdRadarMeterRecord::find($id);
        if (!$record) {
            return $this->error('Form tidak ditemukan.', null, 404);
    ...
    /**
     * DELETE /api/v1/cnsd/radar-meter/{id}
     */
    public function destroy(int $id): JsonResponse
    {
        ...
        return $this->success(null, 'CNSD Radar Meter record deleted successfully');
    }

    /**
     * GET /api/v1/cnsd/radar-meter/years
     */
    public function years(): JsonResponse
    {
        $years = CnsdRadarMeterRecord::selectRaw('EXTRACT(YEAR FROM date)::int AS y')
            ->whereNotNull('date')
            ->groupBy('y')
            ->orderByDesc('y')
            ->pluck('y')
    ...
    private function summarizeRecord(CnsdRadarMeterRecord $r): array
    {
        return [
            'id'                => $r->id,
    ...
    private function detailRecord(CnsdRadarMeterRecord $r): array
    {
        ...
        return [
            ...
            'items' => $r->items->map(fn ($it) => [
                ...
                'kondisi_teknis_tx1'  => $it->kondisi_teknis_tx1,
                'kondisi_teknis_tx2'  => $it->kondisi_teknis_tx2,
                'hasil'               => $it->hasil,
                'keterangan'          => $it->keterangan,
                'sort_order'          => $it->sort_order,
            ])->values()->toArray(),
            'sections_meta' => CnsdRadarMeterTemplate::sectionMeta(),
