...
use App\Exceptions\CnsdAtisMeterDuplicateException;
use App\Exceptions\SignerNotAuthorizedException;
use App\Http\Controllers\Controller;
use App\Http\Requests\Cnsd\CreateCnsdAtisMeterRequest;
use App\Http\Requests\Cnsd\SignCnsdAtisMeterRequest;
use App\Http\Requests\Cnsd\UpdateCnsdAtisMeterRequest;
use App\Models\Cnsd\CnsdAtisMeterRecord;
use App\Services\Cnsd\CnsdActivityLogger;
use App\Services\Cnsd\CnsdAtisMeterService;
use App\Services\Cnsd\CnsdAtisMeterTemplate;
...
class CnsdAtisMeterController extends Controller
{
    use ApiResponse;

    public function __construct(
        protected CnsdAtisMeterService $service,
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $records->through(fn (CnsdAtisMeterRecord $r) => $this->summarize($r));
        return $this->success($records, 'CNSD ATIS Meter records retrieved successfully');
    }

    public function template(): JsonResponse
    {
        return $this->success([
            'form_type' => 'ATIS-METER',
            'facility'  => 'ATIS',
            'sections'  => CnsdAtisMeterTemplate::sections(),
        ], 'ATIS Meter template retrieved successfully');
    }

    public function years(): JsonResponse
    {
        $years = CnsdAtisMeterRecord::selectRaw('EXTRACT(YEAR FROM date)::int AS y')
    ...
    public function store(CreateCnsdAtisMeterRequest $request): JsonResponse
    {
        $user = Auth::user();
        ...
        } catch (CnsdAtisMeterDuplicateException $e) {
        ...
        try {
            $this->activityLogger->logMeterReadingCreated($record, 'ATIS', '/cnsd/atis-meter', $user);
        } catch (\Throwable) { /* non-fatal */ }

        return $this->success($this->detail($record), 'CNSD ATIS Meter record created successfully', 201);
    }
    ...
    public function show(int $id): JsonResponse
    {
        ...
        return $this->success($this->detail($record), 'CNSD ATIS Meter record retrieved successfully');
    }

    public function update(UpdateCnsdAtisMeterRequest $request, int $id): JsonResponse
    {
        $record = CnsdAtisMeterRecord::find($id);
        if (!$record) return $this->error('Form tidak ditemukan.', null, 404);
        $user = Auth::user();
        if (!$user || (!$user->isAdmin() && !$user->isManager() && !$user->isSupervisor() && !$user->isTeknisi())) {
            return $this->error('Unauthorized.', null, 403);
        ...
        return $this->success($this->detail($record), 'CNSD ATIS Meter record updated successfully');
    }

    public function sign(SignCnsdAtisMeterRequest $request, int $id): JsonResponse
    {
        $record = CnsdAtisMeterRecord::find($id);
        if (!$record) return $this->error('Form tidak ditemukan.', null, 404);
        $user = Auth::user();
    ...
    public function destroy(int $id): JsonResponse
    {
        ...
        return $this->success(null, 'CNSD ATIS Meter record deleted successfully');
    }

    private function summarize(CnsdAtisMeterRecord $r): array
    {
        return [
    ...
    private function detail(CnsdAtisMeterRecord $r): array
    {
        ...
        return [
            ...
            'sections_meta' => CnsdAtisMeterTemplate::sectionMeta(),
