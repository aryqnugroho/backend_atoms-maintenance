...
class TfpTransmitterTxController extends Controller
{
    ...
    public function __construct(protected TfpTransmitterTxService $service) {}
    ...
    private function detailRecord(TfpTransmitterTxRecord $r): array
    {
        ...
        return [
            ...
            'items' => $r->items->map(fn ($it) => ['id' => $it->id, 'parameter_number' => $it->parameter_number, 'parameter_name' => $it->parameter_name, 'unit' => $it->unit, 'panel_tx01' => $it->panel_tx01, 'panel_tx02' => $it->panel_tx02, 'panel_cos_tx03_input' => $it->panel_cos_tx03_input, 'panel_cos_tx03_output' => $it->panel_cos_tx03_output, 'panel_output_ups_tx04' => $it->panel_output_ups_tx04, 'panel_ups_tx07_input' => $it->panel_ups_tx07_input, 'panel_ups_tx07_output' => $it->panel_ups_tx07_output, 'panel_ac_tx06' => $it->panel_ac_tx06, 'ups_piller_input' => $it->ups_piller_input, 'ups_piller_output' => $it->ups_piller_output, 'panel_milat_ru11' => $it->panel_milat_ru11, 'is_disabled_map' => $it->is_disabled_map, 'sort_order' => $it->sort_order])->values()->toArray(),
