...
use App\Http\Requests\Tfp\CreateTfpGlidepathRequest;
use App\Http\Requests\Tfp\SaveTfpGlidepathStructureRequest;
...
class TfpGlidepathController extends Controller
{
    use ApiResponse;
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $records->through(fn (TfpGlidepathRecord $r) => $this->summarizeRecord($r));
        return $this->success($records, 'TFP Glide Path records retrieved successfully');
    }
    ...
    public function template(): JsonResponse
    {
        return $this->success([
            'form_type'      => 'GLIDEPATH',
            'location'       => 'GEDUNG GLIDE PATH',
            'columns_config' => TfpGlidepathTemplate::defaultColumnsConfig(),
            'parameters'     => TfpGlidepathTemplate::parameters(),
            'facilities'     => TfpGlidepathTemplate::facilities(),
        ], 'TFP Glide Path template retrieved successfully');
    }

    public function store(CreateTfpGlidepathRequest $request): JsonResponse
    {
        $user = Auth::user();
        ...
        } catch (TfpGlidepathDuplicateException $e) {
    ...
    public function show(int $id): JsonResponse
    {
        ...
        return $this->success($this->detailRecord($record), 'TFP Glide Path record retrieved successfully');
    }

    public function update(UpdateTfpGlidepathRequest $request, int $id): JsonResponse
    {
        $record = TfpGlidepathRecord::find($id);
        if (!$record) return $this->error('Form tidak ditemukan.', null, 404);
        $user = Auth::user();
        if (!$user || (!$user->isAdmin() && !$user->isManager() && !$user->isSupervisor() && !$user->isTeknisi())) {
            return $this->error('Unauthorized.', null, 403);
        ...
        return $this->success($this->detailRecord($record), 'TFP Glide Path record updated successfully');
    }

    public function sign(SignTfpGlidepathRequest $request, int $id): JsonResponse
    {
        $record = TfpGlidepathRecord::find($id);
        if (!$record) return $this->error('Form tidak ditemukan.', null, 404);
        $user = Auth::user();
    ...
    public function destroy(int $id): JsonResponse
    {
        ...
        return $this->success(null, 'TFP Glide Path record deleted successfully');
    }

    public function years(): JsonResponse
    {
        $years = TfpGlidepathRecord::selectRaw('EXTRACT(YEAR FROM date)::int AS y')
    ...
    public function saveStructure(SaveTfpGlidepathStructureRequest $request, int $id): JsonResponse
    {
        $record = TfpGlidepathRecord::find($id);
        if (!$record) return $this->error('Form tidak ditemukan.', null, 404);
    ...
    private function summarizeRecord(TfpGlidepathRecord $r): array
    {
        return [
    ...
    private function detailRecord(TfpGlidepathRecord $r): array
    {
        ...
        return [
            ...
            'location'       => $r->location,
            'columns_config' => $r->columns_config ?: TfpGlidepathTemplate::defaultColumnsConfig(),
            'status'         => $r->status,
