...
use App\Http\Requests\Tfp\CreateTfpRadarRequest;
use App\Http\Requests\Tfp\SaveTfpRadarStructureRequest;
...
class TfpRadarController extends Controller
{
    use ApiResponse;
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $records->through(fn (TfpRadarRecord $r) => $this->summarizeRecord($r));
        return $this->success($records, 'TFP Radar records retrieved successfully');
    }
    ...
    public function template(): JsonResponse
    {
        return $this->success([
            'form_type'      => 'RADAR',
            'location'       => 'GEDUNG RADAR',
            'columns_config' => TfpRadarTemplate::defaultColumnsConfig(),
            'parameters'     => TfpRadarTemplate::parameters(),
            'facilities'     => TfpRadarTemplate::facilities(),
        ], 'TFP Radar template retrieved successfully');
    }

    public function store(CreateTfpRadarRequest $request): JsonResponse
    {
        $user = Auth::user();
        ...
        } catch (TfpRadarDuplicateException $e) {
    ...
    public function show(int $id): JsonResponse
    {
        ...
        return $this->success($this->detailRecord($record), 'TFP Radar record retrieved successfully');
    }

    public function update(UpdateTfpRadarRequest $request, int $id): JsonResponse
    {
        $record = TfpRadarRecord::find($id);
        if (!$record) return $this->error('Form tidak ditemukan.', null, 404);
        $user = Auth::user();
        if (!$user || (!$user->isAdmin() && !$user->isManager() && !$user->isSupervisor() && !$user->isTeknisi())) {
            return $this->error('Unauthorized.', null, 403);
        ...
        return $this->success($this->detailRecord($record), 'TFP Radar record updated successfully');
    }

    public function sign(SignTfpRadarRequest $request, int $id): JsonResponse
    {
        $record = TfpRadarRecord::find($id);
        if (!$record) return $this->error('Form tidak ditemukan.', null, 404);
        $user = Auth::user();
    ...
    public function destroy(int $id): JsonResponse
    {
        ...
        return $this->success(null, 'TFP Radar record deleted successfully');
    }

    public function years(): JsonResponse
    {
        $years = TfpRadarRecord::selectRaw('EXTRACT(YEAR FROM date)::int AS y')
    ...
    public function saveStructure(SaveTfpRadarStructureRequest $request, int $id): JsonResponse
    {
        $record = TfpRadarRecord::find($id);
        if (!$record) return $this->error('Form tidak ditemukan.', null, 404);
    ...
    private function summarizeRecord(TfpRadarRecord $r): array
    {
        return [
    ...
    private function detailRecord(TfpRadarRecord $r): array
    {
        ...
        return [
            ...
            'location'       => $r->location,
            'columns_config' => $r->columns_config ?: TfpRadarTemplate::defaultColumnsConfig(),
            'status'         => $r->status,
