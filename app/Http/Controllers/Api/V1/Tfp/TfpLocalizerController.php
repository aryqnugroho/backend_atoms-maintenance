...
use App\Http\Requests\Tfp\CreateTfpLocalizerRequest;
use App\Http\Requests\Tfp\SaveTfpLocalizerStructureRequest;
use App\Http\Requests\Tfp\SignTfpLocalizerRequest;
use App\Http\Requests\Tfp\UpdateTfpLocalizerRequest;
use App\Models\Tfp\TfpLocalizerRecord;
use App\Services\Tfp\TfpActivityLogger;
use App\Services\Tfp\TfpLocalizerService;
use App\Services\Tfp\TfpLocalizerTemplate;
...
class TfpLocalizerController extends Controller
{
    use ApiResponse;

    public function __construct(
        protected TfpLocalizerService $service,
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $records->through(fn (TfpLocalizerRecord $r) => $this->summarizeRecord($r));
        return $this->success($records, 'TFP Localizer records retrieved successfully');
    }
    ...
    public function template(): JsonResponse
    {
        return $this->success([
            'form_type'      => 'LOCALIZER',
            'location'       => 'GEDUNG LOCALIZER',
            'columns_config' => TfpLocalizerTemplate::defaultColumnsConfig(),
            'parameters'     => TfpLocalizerTemplate::parameters(),
            'facilities'     => TfpLocalizerTemplate::facilities(),
        ], 'TFP Localizer template retrieved successfully');
    }

    public function store(CreateTfpLocalizerRequest $request): JsonResponse
    {
        $user = Auth::user();
        ...
        } catch (TfpLocalizerDuplicateException $e) {
        ...
        try {
            $this->activityLogger->appendLogbookNote(
                'Performance Check Gedung Localizer',
        ...
        return $this->success($this->detailRecord($record), 'TFP Localizer record created successfully', 201);
    }
    ...
    public function show(int $id): JsonResponse
    {
        ...
        return $this->success($this->detailRecord($record), 'TFP Localizer record retrieved successfully');
    }

    public function update(UpdateTfpLocalizerRequest $request, int $id): JsonResponse
    {
        $record = TfpLocalizerRecord::find($id);
        if (!$record) return $this->error('Form tidak ditemukan.', null, 404);
        $user = Auth::user();
        if (!$user || (!$user->isAdmin() && !$user->isManager() && !$user->isSupervisor() && !$user->isTeknisi())) {
            return $this->error('Unauthorized.', null, 403);
        ...
        return $this->success($this->detailRecord($record), 'TFP Localizer record updated successfully');
    }

    public function sign(SignTfpLocalizerRequest $request, int $id): JsonResponse
    {
        $record = TfpLocalizerRecord::find($id);
        if (!$record) return $this->error('Form tidak ditemukan.', null, 404);
        $user = Auth::user();
    ...
    public function destroy(int $id): JsonResponse
    {
        ...
        return $this->success(null, 'TFP Localizer record deleted successfully');
    }

    public function years(): JsonResponse
    {
        $years = TfpLocalizerRecord::selectRaw('EXTRACT(YEAR FROM date)::int AS y')
    ...
    public function saveStructure(SaveTfpLocalizerStructureRequest $request, int $id): JsonResponse
    {
        $record = TfpLocalizerRecord::find($id);
        if (!$record) return $this->error('Form tidak ditemukan.', null, 404);
    ...
    private function summarizeRecord(TfpLocalizerRecord $r): array
    {
        return [
    ...
    private function detailRecord(TfpLocalizerRecord $r): array
    {
        ...
        return [
            ...
            'location'       => $r->location,
            'columns_config' => $r->columns_config ?: TfpLocalizerTemplate::defaultColumnsConfig(),
            'status'         => $r->status,
