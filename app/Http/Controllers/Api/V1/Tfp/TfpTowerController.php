...
use App\Http\Requests\Tfp\SaveTfpTowerStructureRequest;
...
class TfpTowerController extends Controller
{
    ...
    public function saveStructure(SaveTfpTowerStructureRequest $request, int $id): JsonResponse
    {
        $record = TfpTowerRecord::find($id);
        if (!$record) return $this->error('Form tidak ditemukan.', null, 404);
    ...
    private function detailRecord(TfpTowerRecord $r): array
    {
        ...
        return [
            ...
            'location'       => $r->location,
            'columns_config' => $r->columns_config ?: TfpTowerTemplate::defaultColumnsConfig(),
            'status'         => $r->status,
