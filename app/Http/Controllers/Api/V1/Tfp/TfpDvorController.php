...
use App\Http\Requests\Tfp\SaveTfpDvorStructureRequest;
...
class TfpDvorController extends Controller
{
    ...
    public function __construct(
        protected TfpDvorService $service,
    ...
    public function saveStructure(SaveTfpDvorStructureRequest $request, int $id): JsonResponse
    {
        $record = TfpDvorRecord::find($id);
        if (!$record) return $this->error('Form tidak ditemukan.', null, 404);
    ...
    private function detailRecord(TfpDvorRecord $r): array
    {
        ...
        return [
            ...
            'location'       => $r->location,
            'columns_config' => $r->columns_config ?: TfpDvorTemplate::defaultColumnsConfig(),
            'status'         => $r->status,
