...
use App\Http\Requests\GroundCheck\CreateGroundCheckLlzRequest;
use App\Http\Requests\GroundCheck\SignGroundCheckLlzRequest;
use App\Http\Requests\GroundCheck\UpdateGroundCheckLlzRequest;
use App\Models\GroundCheck\GroundCheckLlzPhoto;
use App\Models\GroundCheck\GroundCheckLlzRecord;
use App\Services\GroundCheck\GroundCheckLlzCurveTemplate;
use App\Services\GroundCheck\GroundCheckLlzService;
use App\Services\GroundCheck\GroundCheckLlzTemplate;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;

class GroundCheckLlzController extends Controller
{
    public function __construct(
        protected GroundCheckLlzService $service,
    ) {}
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $items = $records->getCollection()->map(function (GroundCheckLlzRecord $r) {
            return [
                'id'                  => $r->id,
    ...
    {
        $years = GroundCheckLlzRecord::query()
    ...
    public function template(): JsonResponse
    {
        return response()->json([
            'success' => true,
            'data'    => [
                'items'        => GroundCheckLlzTemplate::items(),
                'curve_points' => GroundCheckLlzCurveTemplate::points(),
            ],
        ]);
    }

    public function store(CreateGroundCheckLlzRequest $request): JsonResponse
    {
        $user = Auth::user();
        ...
        try {
            ...
            return response()->json([
                'success' => true,
                'message' => 'Ground Check LLZ berhasil dibuat.',
    ...
    public function update(UpdateGroundCheckLlzRequest $request, int $id): JsonResponse
    {
        $record = GroundCheckLlzRecord::find($id);
    ...
    public function sign(SignGroundCheckLlzRequest $request, int $id): JsonResponse
    {
        $record = GroundCheckLlzRecord::with('technicians')->find($id);
    ...
    private function formatDetail(GroundCheckLlzRecord $record): array
    {
        return [
            'id'                   => $record->id,
            ...
            'curve_facility'       => $record->curve_facility,
            'curve_merk'           => $record->curve_merk,
            'curve_ident_freq'     => $record->curve_ident_freq,
            'curve_jarak_ant'      => $record->curve_jarak_ant,
            ...
            'curve_points'         => $record->curvePoints->map(fn ($p) => [
                'id'              => $p->id,
                'side'            => $p->side,
                'jarak_m'         => (float) $p->jarak_m,
                'degrees'         => (float) $p->degrees,
                'tx1_ddm_pct'     => $p->tx1_ddm_pct !== null ? (float) $p->tx1_ddm_pct : null,
                'tx1_ddm_ua'      => $p->tx1_ddm_ua !== null ? (float) $p->tx1_ddm_ua : null,
                'tx1_sum_pct'     => $p->tx1_sum_pct !== null ? (float) $p->tx1_sum_pct : null,
                'tx1_mod_90hz'    => $p->tx1_mod_90hz !== null ? (float) $p->tx1_mod_90hz : null,
                'tx1_mod_150hz'   => $p->tx1_mod_150hz !== null ? (float) $p->tx1_mod_150hz : null,
                'tx1_rf_level_db' => $p->tx1_rf_level_db !== null ? (float) $p->tx1_rf_level_db : null,
                'tx2_ddm_pct'     => $p->tx2_ddm_pct !== null ? (float) $p->tx2_ddm_pct : null,
                'tx2_ddm_ua'      => $p->tx2_ddm_ua !== null ? (float) $p->tx2_ddm_ua : null,
                'tx2_sum_pct'     => $p->tx2_sum_pct !== null ? (float) $p->tx2_sum_pct : null,
                'tx2_mod_90hz'    => $p->tx2_mod_90hz !== null ? (float) $p->tx2_mod_90hz : null,
                'tx2_mod_150hz'   => $p->tx2_mod_150hz !== null ? (float) $p->tx2_mod_150hz : null,
                'tx2_rf_level_db' => $p->tx2_rf_level_db !== null ? (float) $p->tx2_rf_level_db : null,
                'sort_order'      => $p->sort_order,
            ])->toArray(),
            'photos'               => $record->photos->map(fn (GroundCheckLlzPhoto $p) => [
                'id'              => $p->id,
                'url'             => $p->url,
                'caption'         => $p->caption,
    ...
    {
        $record = GroundCheckLlzRecord::with('photos')->find($id);
        if (!$record) {
        ...
        $record->load(['photos', 'items', 'curvePoints', 'technicians', 'manager', 'supervisor']);
        return response()->json([
            'success' => true,
    ...
    public function updatePhoto(Request $request, int $id, int $photoId): JsonResponse
    {
        ...
        $photo = GroundCheckLlzPhoto::where('ground_check_llz_record_id', $id)->find($photoId);
        if (!$photo) {
