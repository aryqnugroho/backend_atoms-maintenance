...
/**
 * CnsdVccsMeterRecord — header for one CNSD VCCS Meter Reading form (VCCS LES).
 */
class CnsdVccsMeterRecord extends Model
{
    use HasSignature, SoftDeletes;

    protected $table = 'cnsd_vccs_meter_records';
    ...
    public const FORM_TYPES  = ['VCCS-METER'];
    ...
    {
        return $this->hasMany(CnsdVccsMeterTechnician::class, 'vccs_meter_record_id')
            ->orderBy('sort_order')
            ->orderBy('id');
    }
    ...
    {
        return $this->hasMany(CnsdVccsMeterItem::class, 'vccs_meter_record_id')
            ->orderBy('sort_order')
            ->orderBy('id');
    }
