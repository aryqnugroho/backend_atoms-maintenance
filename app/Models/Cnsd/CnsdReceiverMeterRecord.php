...
class CnsdReceiverMeterRecord extends Model
{
    use SoftDeletes, HasSignature;

    protected $table = 'cnsd_receiver_meter_records';
    ...
    {
        return $this->hasMany(CnsdReceiverMeterTechnician::class, 'receiver_meter_record_id')
            ->orderBy('sort_order');
    }

    public function items(): HasMany
    {
        return $this->hasMany(CnsdReceiverMeterItem::class, 'receiver_meter_record_id')
            ->orderBy('sort_order');
    }
    ...
    {
        // Check manager signature if required
        if ($this->manager_name && empty($this->manager_signature)) {
            return false;
        }
        // Check supervisor signature if required
        if ($this->supervisor_name && empty($this->supervisor_signature)) {
            return false;
        }
        // Check all technician signatures
    ...
    public function isShiftEnded(): bool
    {
        ...
        if ($this->shift_type === 'malam') {
            $shiftDate = $this->date->addDay()->format('Y-m-d');
        }
