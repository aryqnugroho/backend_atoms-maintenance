...
/**
 * CnsdVccsFreqMeterRecord — header for one CNSD VCCS Frequentis Meter Reading.
 */
class CnsdVccsFreqMeterRecord extends Model
{
    use HasSignature, SoftDeletes;

    protected $table = 'cnsd_vccs_freq_meter_records';
    ...
    public const FACILITIES  = ['VCCS'];
    public const FORM_TYPES  = ['VCCS-FREQ-METER'];
    ...
    public function managerSigner(): BelongsTo      { return $this->belongsTo(LocalUser::class, 'manager_signed_by'); }
    public function supervisorSigner(): BelongsTo   { return $this->belongsTo(LocalUser::class, 'supervisor_signed_by'); }
    public function creator(): BelongsTo            { return $this->belongsTo(LocalUser::class, 'created_by_id'); }

    public function technicians(): HasMany
    {
        return $this->hasMany(CnsdVccsFreqMeterTechnician::class, 'vccs_freq_meter_record_id')
            ->orderBy('sort_order')->orderBy('id');
    }

    public function items(): HasMany
    {
        return $this->hasMany(CnsdVccsFreqMeterItem::class, 'vccs_freq_meter_record_id')
            ->orderBy('sort_order')->orderBy('id');
    }
    ...
    public function isComplete(): bool
    {
        ...
        $unsignedTech = $this->technicians()->whereNull('technician_signature')->exists();
        return !$unsignedTech;
    }
