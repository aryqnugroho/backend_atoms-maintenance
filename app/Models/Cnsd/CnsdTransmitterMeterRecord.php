...
/**
 * CnsdTransmitterMeterRecord — header for one CNSD Transmitter Meter Reading form.
 *
 * Modeled after CnsdAmscMeterRecord. Lives in its own table so that
 * Transmitter-specific columns stay isolated from other CNSD modules.
 */
class CnsdTransmitterMeterRecord extends Model
{
    use HasSignature, SoftDeletes;

    protected $table = 'cnsd_transmitter_meter_records';
    ...
    {
        return $this->hasMany(CnsdTransmitterMeterTechnician::class, 'transmitter_meter_record_id')
            ->orderBy('sort_order')
            ->orderBy('id');
    }
    ...
    {
        return $this->hasMany(CnsdTransmitterMeterItem::class, 'transmitter_meter_record_id')
            ->orderBy('sort_order')
            ->orderBy('id');
    }
