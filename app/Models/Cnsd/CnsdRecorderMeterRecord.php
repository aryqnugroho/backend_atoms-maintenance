...
/**
 * CnsdRecorderMeterRecord — header for one CNSD Recorder Meter Reading form.
 *
 * Modeled after CnsdRadarMeterRecord. Lives in its own table so that
 * Recorder-specific columns (form_code FORM C-3, merk ATIS - UHER, type
 * VC - MDx, serial_number 51) stay isolated from Radar/EQ-1.
 *
 * Signature contract (mirrors EQ-1 + Radar):
 ...
 *   - Technician signatures live on cnsd_recorder_meter_technicians.
 ...
 */
class CnsdRecorderMeterRecord extends Model
{
    use HasSignature, SoftDeletes;

    protected $table = 'cnsd_recorder_meter_records';
    ...
    public const FACILITIES  = ['RECORDER'];
    public const FORM_TYPES  = ['RECORDER-METER'];
    ...
    {
        return $this->hasMany(CnsdRecorderMeterTechnician::class, 'recorder_meter_record_id')
            ->orderBy('sort_order')
            ->orderBy('id');
    }
    ...
    {
        return $this->hasMany(CnsdRecorderMeterItem::class, 'recorder_meter_record_id')
            ->orderBy('sort_order')
            ->orderBy('id');
    }
